...
const Task = ({ task, onDelete, onToggleComplete, onEdit }) => {
  ...
  return (
    <div className=' bg-white-100 border-none  rounded-lg p-3 m-2 shadow-xl w-[300px] flex flex-col justify-center hover:shadow-2xl'>
      <div className='p-3 '>
      ...
      <div className=' flex justify-end '>
        {!task.completed && (
          <button className='bg-green-300 p-1 rounded-md m-1  pt-1 pb-1 pr-2 pl-2 font-medium hover:bg-green-200 hover:text-white' onClick={() => onToggleComplete(task.id)}>
            Mark Complete
          </button>
        )}
        ...
        <button className='bg-red-400 p-1 rounded-md  m-1  pt-1 pb-1 pr-2 pl-2 font-medium hover:bg-red-300 hover:text-white' onClick={() => onDelete(task.id)}>Delete</button>
      </div>
      {isEditing && (
        <div className=' p-3'>
          <input
